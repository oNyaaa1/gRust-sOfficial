...
local function zSetHealth(icon, name, length, x, y, col)
    ...
    local lgnth = 180
    draw.RoundedBox(4, Hud.Posx + x, Hud.Posy + y, lgnth + 30, 26, Color(160, 152, 140, 150))
    ...
    draw.DrawText(name, "GRustHUDLOW", Hud.Posx + x + 30, Hud.Posy + y, Color(255, 255, 255), TEXT_ALIGN_LEFT)
end

hook.Add("HUDPaint", "MrRustHud", function()
    local ply = LocalPlayer()
    if not IsValid(ply) then return end
    zSetHealth(health, ply:Health(), ply:Health() / ply:GetMaxHealth(), 1, 1, Color(136, 179, 58, 150))
    zSetHealth(water, ply:GetThirst(), ply:GetThirst() / ply:GetMaxThirst(), 1, 30, Color(69, 148, 205, 150))
    zSetHealth(food, ply:GetHunger(), ply:GetHunger() / ply:GetMaxHunger(), 1, 60, Color(193, 109, 53, 150))
end)
